...
class GmailOAuthService {
  static String get clientId => dotenv.env['GMAIL_CLIENT_ID'] ?? '';
  ...
  static Future<String> _buildAuthUrl({required bool forService}) async {
    ...
    final params = {
      ...
      'response_type': 'code',
      'scope': scope,
      'state': forService ? 'service-gmail' : 'gmail',
      'access_type': 'offline',
      'prompt': 'consent'
    };
    ...
    return Navigator.of(context).push<String>(
      MaterialPageRoute(
        ...
        builder: (context) => _OAuthWebView(
            authUrl: authUrl, forService: forService, providerName: 'Gmail'),
      ),
    );
  }
}
...
class _OAuthWebViewState extends State<_OAuthWebView> {
  ...
  void _checkForCode(String url) {
    ...
    // HTTP callback from backend (hosted URL)
